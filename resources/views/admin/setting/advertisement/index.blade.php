    ...
    <main class="main-content">
        <div class="theme-card">
            <div class="theme-card-header d-flex justify-content-between">
                <h6 class="theme-card-title">{{__('app.advertisement').' '.__('app.information').' '.__('app.here')}}</h6>
                <a href="{{route('advertisement.create')}}" class="btn btn-success btn-sm">{{__('app.add')}}</a>
            </div>
                        ...
                        <tbody>
                           @foreach($advertisements as $add)
                            <tr class="text-center">
                                <td>{{$add->name}}</td>
                                <td>
                                    <img  width='60px' src="{{asset('product/'.$add->image)}}"/>
                                </td>

                                <td class="text-center">
                                    <a href="{{route('advertisement.edit',[$add->id])}}" class="btn btn-success btn-sm">{{__('app.edit')}}</a>
                                    <a href="javascript:void(0);" onclick="deleteItem('{{route('advertisement.delete',$add->id)}}')"   class="btn btn-danger btn-sm">{{__('app.delete')}}</a>
                                </td>
                            </tr>
                           @endforeach
                        </tbody>
